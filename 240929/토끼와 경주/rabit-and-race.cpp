     ...
     if(cmd==200){ //경주 진행 
        ...
        //  cout<<"round"<<kk<<'\n';
         ...
         int add=select.r+select.c+2;
          ...
          total+=add; 
        // 
        //   cout<<select.number<<" "<<dist[select.number].second+total<<"점\n";
         //  cout<<pq.top().number<<" "<<dist[pq.top().number].second+total<<"점\n";
        }
        
        //점프한 토끼 하나 s 더해주기 
        dist[pq2.top().number].second+=s;
        //여기서 갱신해줘야할듯... 
     //   cout<<pq2.top().number<<" "<<dist[pq2.top().number].second+total<<'\n';
    }
    if(cmd==300){ //이동거리 l배 
        cin>>num>>l;
        dist[num].first*=l; 
